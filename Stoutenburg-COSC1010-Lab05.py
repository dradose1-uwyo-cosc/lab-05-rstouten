...
# create empty variable for both temp
max_temp = 0
min_temp = 0
#pretend like 1st item is the list is max, set max temp equal
if len(max_temps) > 0:
    max_temp = max_temps[0]
#loop through max temps and if a value is greater than the current max temp then it will replace it
for temp in max_temps:
    if temp > max_temp:
        max_temp = temp
#pretend like the 1st item is the min
if len(min_temps) > 0:
    min_temp = min_temps[0]
#replace min_temp value with any value lower than it
...
for value in numbers: #loop through all numbers
    if value < 0: 
        neg_count+=1 #when value is less than zero add 1 do the count
    elif value == 0:
        zero_count+=1 #same thing
    else:
        pos_count+=1 #if it's not less than zero or zero it's pos
...
for num in numbers: #loop through numbers
    if num > 0: #when num is greater than zero add to pos sum
        pos_sum+=num
    else:
        neg_sum+=num #if it isn't greater than zero it's zero or negative
